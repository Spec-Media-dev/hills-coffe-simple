 ...
 *
 * The "current contract" block documents what is live today and must keep
 * passing. The "extension" block asserts the owner-approved additions and is
 * skipped until the owner applies
 *   specs/001-platform-implementation-spec/migrations/
 *     P1-T02_admin_list_users_extension.sql
 * after which `HILLS_ADMIN_LIST_USERS_EXTENDED=1` enables it.
 *
...
const extensionApplied = process.env.HILLS_ADMIN_LIST_USERS_EXTENDED === "1";
...
suite("P1-T02 admin_list_users() read path", () => {
  ...
  (extensionApplied ? describe : describe.skip)(
    "owner-approved extension (set HILLS_ADMIN_LIST_USERS_EXTENDED=1 once applied)",
    () => {
      ...
      it("searches by email, case-insensitively and partially", async () => {
        ...
        expect(
          ((data ?? []) as UserRow[]).some((r) => r.id === active.id),
        ).toBe(true);
      });
      ...
      it("still refuses every non-admin caller after the extension", async () => {
        expect(
          (await active.client.rpc("admin_list_users")).error?.message,
        ).toBe("Forbidden");
